...
create_virtualenv()
{
    ...
    [ ! -d ${PYTHONDIR} ] && virtualenv3 ${PYTHONDIR}
...
User=$KLIPPER_USER
RemainAfterExit=yes
ExecStartPre=!/usr/bin/install -m 755 -o $KLIPPER_USER -d /run/klipper
ExecStart=${PYTHONDIR}/bin/python ${SRCDIR}/klippy/klippy.py ${HOME}/printer.cfg -l /run/klipper/klippy.log -I /run/klipper/printer
EOF
